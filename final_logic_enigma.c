...
char current_plugboard_config[256]="";
...
void initialize_plugboard(const char *pairs) {
    ...
    strncpy(current_plugboard_config,pairs,sizeof(current_plugboard_config));
    current_plugboard_config[sizeof(current_plugboard_config)-1]='\0';
...
void update_plugboard(){
    char choice;
    char new_config[256];
    printf( "current plugboard:%s \n", current_plugboard_config);
    printf("change plugboard? (y/n)");
    choice=getchar();
    int c;
    while((c=getchar())!='\n'&& c!=EOF);
    if (choice=='y'){
...
void encrypt_message(const char *input, char *output) {
    ...
    while (input[i] != '\0') {
        if(intput[i] !=' '){
            ...
            output[i]=encrypt_char(intput[i]);
            i++;
        }
